...
public List<Evento> getEventos(){
    ...
    StatementResult result = session.run("match (n:Evento) return n.titulo as titulo, n.descricao as descricao, ID(n) as id");
...
public boolean inserirEvento(Evento e){
    ...
    Con c = new Con();
    Session session =  c.conecta(); // chama o metodo de conectar
    StatementResult result = session.run("CREATE (u:Evento {titulo:'"+ titulo +"', descricao:'"+descricao +"'})");
    c.encerraConexao();
...
public Evento buscarEvento(int id){
    Con c = new Con();
    Session session = c.conecta(); // chama o metodo para conectar
    StatementResult result = session.run("match (e:Evento) where ID(e)= "+ id+" return e.descricao as descricao, e.titulo as titulo,ID(e) as id ");
    Record record = result.next();
    Evento e = new Evento();
...
public boolean atualizarEvento(Evento e){
    ...
    int id = e.getId();
    Session session = c.conecta(); // chama o metodo para conectar
    StatementResult result = session.run("match (e:Evento) where ID(e)=" +id+" set e.titulo = '"+ e.getTitulo()+"', e.descricao ='"+e.getDescricao()+"' return e");
    c.encerraConexao();
    if(result!=null){
        return true;
    }else{
